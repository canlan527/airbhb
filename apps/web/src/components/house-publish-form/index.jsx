...
function filesFromInitialImages(imageUrls = defaultImageUrls) {
  return imageUrls.map((url, index) => ({
    uid: `initial-${index}-${url}`,
    name: `房源图片-${index + 1}.jpg`,
    status: 'done',
    url
  }))
...
const HousePublishForm = memo(({
  ...
  async function handleFinish(values) {
    ...
    try {
      ...
      messageApi.error(error?.message || errorText)
    } finally {
    ...
    <div className={`publish-house-page fullstack-page fullstack-page--form${embedded ? ' is-embedded' : ''}`}>
      ...
      <div className="publish-hero fullstack-hero">
        ...
        <div className="publish-flow">
          {flowSteps.map(item => <span key={item}>{item}</span>)}
        </div>
      </div>
      ...
      <Card className="publish-card fullstack-card">
        <Form form={form} layout="vertical" onFinish={handleFinish} initialValues={initialValues}>
          ...
          <div className="form-actions">
            ...
            <Button type="primary" htmlType="submit" loading={submitLoading}>{submitText}</Button>
          </div>
        </Form>
      </Card>
    </div>
