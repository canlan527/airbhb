...
```env
REACT_APP_API_BASE_URL=http://your-api-host/airbnb/api
```
...
线上不要配置 `REACT_APP_API_BASE_URL`，除非你明确希望浏览器直接请求该地址。保持它为空时，生产环境会使用 `/api/proxy`，由 Vercel 服务端完成转发。
...
- 使用 Vite 替换 Create React App 和 CRACO，提升开发启动速度和构建体验
...
- 当前项目可以正常构建，但仍有一些 ESLint warning，主要包括未使用变量、图片缺少 `alt`、Hook 依赖提示等。这些 warning 不会阻塞部署，后续可以继续清理。
